...
func checkIfFloor():
	...
	elif cR.canLedgeGrab and LedgeGrabState.can_grab_ledge():
...
func move(delta : float):
	if !cR.is_active:
		...
		# Cancel desired move speed accumulation if the timer has elapsed
		if cR.hitGroundCooldown <= 0: 
			cR.desiredMoveSpeed = cR.velocity.length()
		
		# Set to ensure the character doesn't exceed max speed
